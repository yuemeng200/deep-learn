...
上一章我们解决了创建型的问题——"怎么创建对象"。本章进入设计模式的第二个分类——**结构型**，解决的核心问题是：**如何组织对象之间的关系，让业务逻辑不被基础设施绑架**。这是"创建 → 组装 → 运转"三步中的第二步——砖造好了，怎么砌。Repository 模式和依赖注入不是 GoF 的经典分类，但它们是 Go Web 开发中最高频、最实用的结构型实践——正是"低成本修改和扩展"这个核心本质在代码结构层面的直接体现。
